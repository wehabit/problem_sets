...
tail(d)
d$Subject
# filter out the subject 2 and 23
d_cleaned <- filter(d, !(Subject %in% c(2,23)))
d_cleaned$Subject
...
```{r}
# print out the d_clean and d to see the differences
#print("d_cleaned$DoNotUse")
#d_cleaned$DoNotUse
#print("d$DoNotUse")
#d$DoNotUse

# filter out d via use of DoNotUse field
filtered_d = d %>% 
  filter(is.na(d$DoNotUse)) # your code here: exclude subjects that are marked as "DoNotUse"
...
rating_hyp_long_d = rating_hyp_long_d %>%
  ...
  mutate(IsRecall = factor(IsRecall, levels=c(F, T), labels=c("Music", "Recall"))) %>% 
  mutate(ConfrontationalGame = factor(ConfrontationalGame, levels=c(F, T), labels=c("Confrontational", "Nonconfrontational"))) 
#rating_hyp_long_d
```
...
performance_diff_d = performance_hyp_long_d %>%
  mutate(WithMusic = factor(WithMusic, levels=c(F, T), labels=c("PreMusic", "PostMusic"))) %>% # first, tweak the variable so our code is easier to read. Lable the F abd Tl 
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarize(MeanImprovementScore = mean(ImprovementScore, na.rm=T), sdImprovementScore = sd(ImprovementScore, na.rm=T), seImprovementScore = std.error(ImprovementScore, na.rm=T))
...
ggplot(performance_diff_summary_d, aes(x=ConfrontationalGame, y= MeanImprovementScore, fill=MusicCondition )) +
    geom_bar(position="dodge", stat="identity") + 
   geom_errorbar(aes(ymin = MeanImprovementScore - seImprovementScore, ymax = MeanImprovementScore + seImprovementScore), width = .085,position=position_dodge(1.), stat="identity") +  
   scale_fill_brewer(palette="Set1") 
